...
function createListing() {
    ...
    data.marketplaceListings.push({
        id: data.marketplaceListings.length + 1,
        title: title,
        description: desc,
        price: price,
        seller: data.currentUser.id,
        category: cat,
        createdAt: Date.now()
    });
...
function renderListings() {
    ...
    let filtered = data.marketplaceListings.filter(item => {
    ...
    grid.innerHTML = filtered.map(item => {
        const seller = data.users.find(u => u.id === item.seller);
        return `
...
function showDetail(id) {
    const item = data.marketplaceListings.find(l => l.id === id);
    if (!item) return;
